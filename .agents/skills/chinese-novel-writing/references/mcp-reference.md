...
| 用户直接指定章节 | `source_sections({work_id, limit?, cursor?})` → `source_paragraphs({work_id, section_id, limit?, cursor?, format?})` | 核对重名章节的 ordinal，空章节无正文；段落结果已含完整 text 与稳定 id |
...
标注列表、说明搜索及标注语义召回默认只返回 active 标注；annotation_get 若返回 withdrawn，不再依据该标注说明创作，但仍可独立阅读原文。查询后资产可能被其他 AI 撤回，应以当前 get 为准；旧回执没有 status 不等于当前有效。恢复后的标注可能尚未完整索引，继续按实际 coverage 判断。写作流程不自行修订标签、撤回或恢复资产。
...
- 证据足够时停止：依据入口 Skill 的任务标准判断；局部写法的证据充分，不代表已读懂人物互动或节奏展开。满足当前参考需要后，后续查询只重复相同证据时没有必要继续。
