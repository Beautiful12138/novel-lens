...
参考查询支持风格导航、标签/实体字面查询、标注过滤、关系查询、原文与写法说明关键词搜索及按坐标读取原文；关键词能力需要 PGroonga，语义检索另需 pgvector 与本机 embedding。当前版本数据库须迁移至 0010，支持标注撤回、作品屏蔽及默认检索排除；跨层统一 Search 尚未提供。实际 AI 宿主触发、文学效果与读取成本仍待后续联调。未安装到全局目录或修改宿主配置。

## 作品屏蔽与物理删除

数据库升级至 `0010` 后，可在 HTTP 接口文档 `/docs` 调用，或由 WebUI 接入。管理操作本期不提供 MCP 写入工具。
...
这是开发期默认值调整；依赖完整字段的调用需显式 format="full"。修改标注前也应读取 full，避免直接把精简范围或不完整对象用于覆盖写入。写入结果、asset_write_get 历史回执及其他资产接口不精简。此调整无需新增数据库迁移；当前代码须完成 0010 作品管理迁移。
...
先按部署指南配置 `NOVEL_LENS_EMBEDDING_CONFIG` 并运行本机模型，数据库迁移至 0010。`kind=fulltext`（默认）查询全文，`kind=annotation` 查询有效标注引用的原文；两层均不把标注说明或标签混入向量。以下 UUID 占位符须替换为实际 ID；请求 ID 在调用前生成并保留。
