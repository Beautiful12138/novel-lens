...
参考查询支持风格导航、标签/实体字面查询、标注过滤、关系查询、原文与写法说明关键词搜索及按坐标读取原文；关键词能力需要 PGroonga 和迁移 0006；全文与标注原文语义检索需要 pgvector、迁移 0008 与本机 embedding；跨层统一 Search 尚未提供。实际 AI 宿主触发、文学效果与读取成本仍待后续联调。未安装到全局目录或修改宿主配置。
...
当前共提供 46 个工具，其中九个原文工具如下，九个标注与标签工具、十一个实体与关系工具、三个风格导航工具、八个任务与进度工具、两个关键词搜索工具和四个原文语义索引工具见后续章节：
...
先按部署指南配置 `NOVEL_LENS_EMBEDDING_CONFIG` 并运行本机模型，数据库迁移至 0008。`kind=fulltext`（默认）查询全文，`kind=annotation` 查询标注引用的原文；两层均不把标注说明或标签混入向量。以下 UUID 占位符须替换为实际 ID；请求 ID 在调用前生成并保留。
