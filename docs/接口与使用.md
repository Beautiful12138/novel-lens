...
参考查询支持风格导航、标签/实体字面查询、标注过滤、关系查询、原文与写法说明关键词搜索及按坐标读取原文；关键词能力需要 PGroonga 和迁移 0006；全文原文语义检索需要 pgvector、迁移 0007 与本机 embedding。标注原文向量与跨层统一 Search 尚未提供。实际 AI 宿主触发、文学效果与读取成本仍待后续联调。未安装到全局目录或修改宿主配置。
...
当前共提供 46 个工具，其中九个原文工具如下，九个标注与标签工具、十一个实体与关系工具、三个风格导航工具、八个任务与进度工具、两个关键词搜索工具和四个全文语义索引工具见后续章节：
...
先按部署指南配置 `NOVEL_LENS_EMBEDDING_CONFIG` 并运行本机模型，数据库迁移至 0007。当前只支持 `kind=fulltext`，不检索标注说明或标注引用的精选原文层。以下 UUID 占位符须替换为实际 ID；请求 ID 在调用前生成并保留。
...
1. 用 `semantic_index_get({work_id})` 发现 `active` 和 `target`，没有索引时 `state=missing`。状态读取无需模型在线。各代摘要包含 `index_id`、`contract_id`、`generation_status`、`coverage` 和 `last_error`。
2. 明确需要创建或重建时调用 create，返回顶层 `index_id`。相同 request_id 返回原创建快照且 `replayed=true`；接续用原 index_id 调用 build，不重复 create。
3. build 默认最多 4 项，可设 1—4；批次总模型输入不超过 4095 token。新批次使用新 request_id，超时后同 ID、同参数重试。向量、扫描进度与回执同事务提交；失败不发布半批。返回 `batch_ready`、`batch_blocked`、`batch_tokens`、覆盖与 active/target ID。历史回执只代表原批次，当前状态另用 get 查询。
4. `generation_status=ready` 表示完整覆盖并已原子发布；`building` 继续分批，`failed` 修复服务后接续，`partial` 表示扫描结束但存在明确缺口，`superseded` 表示已被新的目标代替代。超长段落不截断；get 指定 index_id 后通过 `blocked` 与 `next_cursor` 查看原因和真实范围，分页 limit 默认 100、最大 100。
