...
验证日期：2026-09-21。本文记录 Spec 10 的模型与运行时实验，不是当前 NovelLens 的安装前置步骤。独立配置验证和启动入口见[部署指南](部署指南.md#ai-协助准备-embedding)。2026-09-22 的全文语义增量已接入该模型，业务验收与剩余边界见 [Spec 10 第 10 节](../specs/10-原文语义检索与Embedding部署.md#10-全文增量验证)。
...
- pgvector 0.8.6 已完成与 PostgreSQL 18.6 / PGroonga 4.0.8 的隔离共存及恢复验证；本次模型实验不包含正式索引业务；后续全文增量已实现，共享库未随代码升级。设计与补充实验见 [Spec 10](../specs/10-原文语义检索与Embedding部署.md#9-索引设计阶段验证)。
