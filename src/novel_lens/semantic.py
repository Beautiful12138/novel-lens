"""显式全文索引：会话锁保护代切换，短事务发布批次，精确余弦查询。"""
...
def lock_key(work_id: UUID) -> int:
    """业务命名空间参与哈希，避免与原有资产锁共享整数键。"""
    raw = hashlib.sha256(f"novel-lens:semantic:fulltext:{work_id}".encode()).digest()[:8]
    return int.from_bytes(raw, "big", signed=True)


class SemanticService:
    """只维护全文派生数据；原文视为不可变，发布前仍核对来源身份与端点归属。"""
    ...
    def _schema(conn: Connection) -> None:
        ...
        if not ready:
            raise ServiceError("SEMANTIC_SCHEMA_NOT_READY", "请先安装 vector 并升级至 0007", 503)

    @contextmanager
    def _locked(self, work_id: UUID) -> Iterator[Connection]:
        ...
        with self.database.engine.connect() as conn:
            ...
            try:
                with conn.begin():
                    ...
                    acquired = bool(
                        conn.execute(
                            text("SELECT pg_try_advisory_lock(:key)"), {"key": lock_key(work_id)}
                        ).scalar()
                    )
                if not acquired:
                ...
                if acquired and not conn.invalidated:
                    try:
                        conn.rollback()
                        conn.execute(
                            text("SELECT pg_advisory_unlock(:key)"), {"key": lock_key(work_id)}
                        )
                        conn.commit()
    ...
    @staticmethod
    def _head(conn: Connection, work_id: UUID) -> dict[str, Any]:
        row = (
            conn.execute(
                text("""
            SELECT active_index_id, target_index_id FROM semantic_index_heads
            WHERE work_id=:work AND kind='fulltext'
        """),
                {"work": work_id},
            )
            .mappings()
    ...
    @staticmethod
    def _index(conn: Connection, work_id: UUID, index_id: UUID) -> dict[str, Any]:
        row = (
            conn.execute(
                text("""
            SELECT * FROM semantic_indexes WHERE id=:id AND work_id=:work AND kind='fulltext'
        """),
                {"id": index_id, "work": work_id},
            )
            .mappings()
            .first()
        )
    ...
    def create(self, request: SemanticCreate) -> SemanticWrite:
        ...
        with self._locked(request.work_id) as conn:
            with conn.begin():
                old = (
                    conn.execute(
                        text("""
            ...
            with conn.begin():
                ...
                head = self._head(conn, request.work_id)
                ...
                conn.execute(
                    ...
                    VALUES (:id,:work,'fulltext',:request,:fingerprint,:contract,:source,
                ...
                """),
                    {
                        ...
                        "cursor": json.dumps({"section": 0, "paragraph": 0, "overlap_start": None}),
                    },
                )
                conn.execute(
                    text("""
                    INSERT INTO semantic_index_heads (work_id, kind, target_index_id)
                    VALUES (:work,'fulltext',:id) ON CONFLICT (work_id,kind)
                    DO UPDATE SET target_index_id=excluded.target_index_id
                """),
                    {"work": request.work_id, "id": identifier},
                )
                result = self._write_result(conn, self._index(conn, request.work_id, identifier))
                conn.execute(
                    text("""
    ...
    @staticmethod
    def _paragraphs(
        conn: Connection, work_id: UUID, cursor: dict[str, Any], *, overlap: bool = False
    ) -> list[dict[str, Any]]:
        ...
        with self._locked(request.work_id) as conn:
            with conn.begin():
                ...
                if (
                    ...
                    or self._head(conn, request.work_id)["target_index_id"] != request.index_id
                ):
            ...
            try:
                ...
                if not scanned and read_next(cursor) is None:
                    scanned = True
                ...
                with conn.begin():
                    ...
                    if status == "ready":
                        updated = conn.execute(
                            ...
                            WHERE work_id=:work AND kind='fulltext' AND target_index_id=:id
                        """),
                            {"id": request.index_id, "work": request.work_id},
                        )
    ...
    def _insert_chunk(
        ...
        key = digest(["fulltext", str(first["section_id"]), str(first["id"]), str(last["id"])])
        conn.execute(
            text("""
            ...
            VALUES (:id,:index,:work,'fulltext',:key,:section,:start,:end,:lo,:hi,:hash,:tokens,
                    :bytes,CAST(:vector AS vector),:reason)
        """),
            {
                "id": uuid4(),
                "index": index["id"],
    ...
    def get(self, request: SemanticGet) -> SemanticStatus:
        ...
        with self.database.engine.connect().execution_options(
            ...
            with c.begin():
                ...
                self._schema(c)
                head = self._head(c, request.work_id)
                summaries = {}
                for label in ("active", "target"):
                    identifier = head[label + "_index_id"]
                    summaries[label] = (
                        self._summary(c, self._index(c, request.work_id, identifier))
                        if identifier
                        else None
                    )
                ...
                result.index = self._summary(c, self._index(c, request.work_id, request.index_id))
                scope = f"semantic-blocked:{request.work_id}:fulltext:{request.index_id}"
                ...
                rows = (
                    c.execute(
                        text("""
                    SELECT * FROM semantic_index_items
                    WHERE index_id=:id AND blocked_reason IS NOT NULL
                    AND id>:after ORDER BY id LIMIT :limit
                """),
                        {"id": request.index_id, "after": after_id, "limit": request.limit + 1},
                    )
                    .mappings()
                    .all()
                )
    ...
    def search(self, request: SemanticSearch) -> SemanticResults:
        ...
        with self.database.engine.connect().execution_options(
            ...
            with c.begin():
                ...
                index = self._index(c, request.work_id, identifier)
                if (
