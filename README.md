...
原文读取、关键词与语义搜索、标注查询默认返回 `compact` 精简格式，保留正文、说明、必要定位和续读信息；需要完整元数据时显式传 `format: "full"`。这是开发期接口调整，已有调用若依赖完整字段，需要同步参数；写入结果与历史回执保持完整。
...
当前使用 0011 空库结构，不迁移旧数据。已有实例应另建空库，重新导入与分析；升级不会自动清空旧库。
...
按宿主规则安装**完整 Skill 目录**，包含 `SKILL.md` 与 `references/`。写作 Skill 会根据创作需要主动选择已连接 NovelLens 中的合适参考，并补读必要原文；独立写作无需强制连接。原文阅读支持 compact 精简格式，工具示例与参数见[接口与使用](docs/接口与使用.md)。
