...
分析资产保留的是可核验的发现与引用。当前搜索使用关键词；自然语言语义检索仍在开发中。
...
默认在自己的环境运行 Python 应用与 PostgreSQL / PGroonga，无需维护者的服务器或账号。
...
| 已实现独立入口 | embedding 配置校验、真实试运行、保存与启动；CPU 已实测，尚未接入业务检索 |
| 待实现 | 原文向量索引、可接续索引构建、语义检索接口；后续 Web UI |
