...
目前提供本机 HTTP / MCP 服务，没有 Web UI；语义检索、embedding 与硬件自动配置仍在设计中。当前使用无需 GPU 或模型下载。服务没有账号鉴权，只允许回环监听。
...
- [功能规格](specs/)
- [语义检索与 embedding 设计草案](specs/10-原文语义检索与Embedding部署.md)：尚未实现，不作为当前安装步骤。
- [开发约定](AGENTS.md)
